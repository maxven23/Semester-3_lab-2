...
int strcomp(string s1, string s2) {
	...
	if (s1[min(s1.length(), s2.length())]) return -1;
	else if (s2[min(s1.length(), s2.length())]) return 1;
	else return 0;
}
...
IDictionary<string, int>* alphabetPtr(string input, int size, string type) {
	...
	if (type == "words") {
		...
		for (int current_page = 1; (current_page <= num_pages); ++current_page) {
			if (current_page == 1) {
				for (int word = 0; (word < first_page) && (words > 0); ++word) {
					output->Add(list->Get(word), current_page, strcomp);
					--words;
				}
			}
			else if ((current_page % 10) != 0) {
				for (int word = 0; (word < page) && (words > 0); ++word) {
					output->Add(list->Get(word + first_page + (current_page - 2) * page + tenthpages * tenth_page), current_page, strcomp);
					--words;
				}
			}
			else {
				for (int word = 0; (word < tenth_page) && (words > 0); ++word) {
					output->Add(list->Get(word + first_page + (current_page - 2 - tenthpages) * page + tenthpages * tenth_page), current_page, strcomp);
					--words;
					++tenthpages;
				}
			}
		...
		cout << list->GetSize() + 1 << " words" << endl;
		int added = 0;
		...
		for (int i = first_page, k = 0; i > 0; ) {
			if ((i - let_list->Get(k)) < 0) {
				...
				for (int m = 0; m < k; m++) {
					output->Add(list->Get(m), 1, strcomp);
				}
				i -= let_list->Get(k);
				for (int n = 0; n < k; n++) {
					list->RemoveAt(0);
		...
		while (len > 0) {
			...
			if ((current_page % 10) != 0) {
				if (let_list->Get(let_list->GetSize() - 1) > page) {
					for (int i = page, k = 0; i > 0; k++) {

						if ((i - let_list->Get(k)) < 0) {
							//cout << "# words: " << k << endl << "curr_page: " << current_page << endl;
							for (int m = 0; m < k; m++) {
								output->Add(list->Get(m), current_page, strcomp);
							}
							i -= let_list->Get(k);
							for (int m = 0; m < k; m++) {
								list->RemoveAt(0);
					...
					for (int i = 0; i < list->GetSize(); i++) {
						output->Add(list->Get(i), current_page, strcomp);
						++added;
					}
					len -= let_list->Get(let_list->GetSize() - 1);
				}
				++current_page;
				//cout << added << " words is used" << endl << len << endl << endl;
			}
			else {
				if (let_list->Get(let_list->GetSize() - 1) > tenth_page) {
